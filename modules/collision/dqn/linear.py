...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        s_shape = (None, state_shape[0], state_shape[1])
        a_shape = (None, )
        r_shape = (None,)
        sp_shape = (None, state_shape[0], state_shape[1])
        done_mask_shape = (None,)
        lr_shape = ()

        self.s = tf.placeholder(tf.float32, s_shape)
        self.a = tf.placeholder(tf.int32, a_shape)
        self.r = tf.placeholder(tf.float32, r_shape)
        self.sp = tf.placeholder(tf.float32, sp_shape)
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        #assert out.get_shape().as_list()==[None,num_actions]
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def add_loss_op(self, q, target_q):
        ...
        # Check all of the shapes
        #assert q_sa.get_shape().as_list() == [None]
        #assert q_samp.get_shape().as_list() == [None]
        #assert self.loss.get_shape().as_list() == []
        ##############################################################
        ######################## END YOUR CODE #######################
